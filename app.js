...
function buildNav() {
  ...
  const items = [
    ...
    { key: 'program-history', label: 'Program History', roles: ['fti','sam_officer','admin'],
...
function navTo(page) {
  ...
  if (page === 'program-history') renderProgramHistory();
  // registry route preserved — uncomment to re-enable Alias Lookup:
  // if (page === 'registry')   renderAliasLookup();
}
...
function tabRow(tabs, activeKey) {
  return `<div class="tab-row">${tabs.map(t =>
    `<button class="btn btn-sm ${t.key===activeKey?'btn-primary':''}" onclick="${t.onclick}" ${t.title?`title="${t.title}"`:''}>${t.label}</button>`
  ).join('')}</div>`;
}
...
function candidateTabs(active) {
  const tabs = [
    ...
    { key: 'capstone',    label: 'IPE',        onclick: 'renderCapstoneForm()', title: 'Independent Practice Evaluation' },
  ];
